...
import { IClubData, IClubState, IClubSwitchers } from 'types/entities/club';
import {
...
import { IFileInfo, IInput, IInputsState } from 'types/interfaces';
import { isValueProvided, simpleUuid } from 'utils/common';
import { mapClubToAPI } from 'utils/entities/club';
...
import styles from './ClubActionsPage.module.scss';
...
export const ClubActionsPage = ({ data }: Props) => {
  ...
  const [switchers, setSwitchers] = useState<IClubSwitchers>({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const stateValues: Partial<IClubData> = {
        ...mainPartition,
        ...expieriencePartition,
        ...contactsPartition,
        ...membersPartition,
      ...
      if (!data) {
        await API.club.create(
          mapClubToAPI(stateValues as IClubData, false),
          profile.id
        );
      ...
      if (e instanceof ServerError) {
        addError('Произошла критическая ошибка при обновлении данных клуба!');
