...
import { mapMethodologyToAPI } from 'utils/entities/methodology';
...
import styles from './MethodologyActionsPage.module.scss';

const CURRENT_STEPS_NUMBER = 4;

type Props = {
  data: IMethodologyData | null;
};

export const MethodologyActionsPage = ({ data }: Props) => {
  ...
  const [switchers, setSwitchers] = useState<IMethodologySwitchers>({
  ...
  const [mainPartition, setMainPartition] = useState<
    IMethodologyState['mainPartition']
  >({
    direction: data?.direction ?? -1, //Направленность
    prevalence: data?.prevalence ?? -1, //Распространенность методики
    activityOrganizationForm: data?.activityOrganizationForm ?? -1, //Форма организации деятельности при реализации технологии/методики
    applicationPeriod: data?.applicationPeriod ?? -1, //Период применения (продолжительность реализации)
    authors: registerInput(data?.authors ?? '', textInputValidator), //Автор(ы) (составитель) технологии/методики, информация о согласовании (при наличии)
    publicationLink: registerInput(
      data?.publicationLink ?? '',
      textInputValidator
    ), //Ссылка на публикацию
    effectivenessStudy: registerInput(
      data?.effectivenessStudy ?? '',
      textInputValidator
    ), //Исследование эффективности или доказательности методики/технологии
    effectivenessStudyLink: registerInput(
      data?.effectivenessStudyLink ?? '',
      textInputValidator
    ), //Ссылка на Исследование эффективности или доказательности методики/технологии
    realizedCycles: registerInput(
      data?.realizedCycles ?? '',
      textInputValidator
  ...
  const [
    ...
    setExpieriencePartition,
  ] = useState<ICommonExpieriencePartitionState>({
  ...
  const [
    contactsPartition,
    setContactsPartition,
  ] = useState<ICommonContactsPartitionState>({
  ...
  const [
    membersPartition,
    setMembersPartition,
  ] = useState<ICommonMembersPartitionState>({
      ...
      case 0: {
        ...
        if (!isNextStepAvailable) {
          ...
          setSelectsErrors({
            ...
            prevalence: mainPartition.prevalence === -1,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const stateValues: Partial<IMethodologyData> = {
        ...mainPartition,
        ...expieriencePartition,
        ...contactsPartition,
        ...membersPartition,
        ...
        prevalence: mainPartition.prevalence,
        activityOrganizationForm: mainPartition.activityOrganizationForm,
        applicationPeriod: mainPartition.applicationPeriod,
        authors: mainPartition.authors.value || null,
        publicationLink: mainPartition.publicationLink.value || null,
        effectivenessStudy: mainPartition.effectivenessStudy.value || null,
      ...
      if (!data) {
        await API.methodology.create(
          mapMethodologyToAPI(stateValues as IMethodologyData, false),
          profile.id
        );
      ...
      if (e instanceof ServerError) {
        addError(
          'Произошла критическая ошибка при обновлении данных методики и технологии!'
        );
