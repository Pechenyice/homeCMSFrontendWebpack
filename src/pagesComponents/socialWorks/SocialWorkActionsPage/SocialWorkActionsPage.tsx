...
import styles from './SocialWorkActionsPage.module.scss';

const CURRENT_STEPS_NUMBER = 4;

type Props = {
  data: ISocialWorkData | null;
};

export const SocialWorkActionsPage = ({ data }: Props) => {
  ...
  const [switchers, setSwitchers] = useState<ISocialWorkSwitchers>({
  ...
  const [mainPartition, setMainPartition] = useState<
    ISocialWorkState['mainPartition']
  >({
    ...
    programType: data?.programType ?? -1, //Вид программы
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const stateValues: Partial<ISocialWorkData> = {
        ...mainPartition,
        ...expieriencePartition,
        ...contactsPartition,
        ...membersPartition,
      ...
      if (!data) {
        await API.socialWork.create(
          mapSocialWorkToAPI(stateValues as ISocialWorkData, false),
          profile.id
        );
      ...
      if (e instanceof ServerError) {
        addError(
          'Произошла критическая ошибка при обновлении данных программы по соц. работе!'
        );
